...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(responsePanel_1.ResponsePanelProvider.viewType, responsePanelProvider));
    ...
    // Register toggle Run Point visibility command
    const toggleRunPointCommand = vscode.commands.registerCommand('pwConsole.toggleRunPointVisibility', async () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            const position = editor.selection.active;
            runPointDecorationManager.toggleRunPointAtPosition(editor, position);
        }
    });
    context.subscriptions.push(toggleRunPointCommand);
