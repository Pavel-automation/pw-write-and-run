...
class LibrariesChangeDetector {
    ...
    readImportLines() {
        ...
        const importLines = [];
        for (const line of testFileContent.split('\n')) {
    ...
    capture() {
        try {
            const lines = this.readImportLines();
            if (lines === null) {
                return;
            }
            this.captureLibraries(lines.slice(0, 5));
        }
        catch (error) {
    ...
    /**
     * Reads all import statements from the top of the test file and returns
     * converted dynamic import() strings for each successfully parsed import.
     * Used for first-run library reload — covers both local and npm imports.
     * Stops scanning at the first non-import, non-blank, non-comment line.
     */
    getInitialImports() {
        ...
        try {
            ...
            for (const trimmed of lines) {
                const lib = new libraries_1.ImportLibrary(trimmed, this.testFileDir);
                if (lib.importSucessfull()) {
                    convertedImports.push(lib.getConvertedImport());
                    (0, utils_1.mylog)('Initial import captured for reload:', trimmed);
                }
                else {
                    (0, utils_1.mylog)('Skipping import (parse/validation failed):', trimmed, lib.getErrorMessage());
                }
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            (0, utils_1.mylog)('Error getting initial imports:', errorMessage);
        }
        return convertedImports;
    }
    backupLibraries() {
